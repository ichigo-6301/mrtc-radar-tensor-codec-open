module mrtc_header_axis_streamer #(
  ...
  input  logic                               rst_n,
  input  logic                               i_start,
  input  logic [(HEADER_BYTES*8)-1:0]        i_header_flat,
  input  logic                               i_header_is_packet_last,
  ...
  localparam int AXIS_WIDTH_SUPPORTED_CHECK =
    1 / (((AXIS_DATA_W == 32) ||
          ...
          (AXIS_DATA_W == 256) ||
          (AXIS_DATA_W == 512)) ? 1 : 0);
  localparam int HEADER_BYTE_ALIGN_CHECK = 1 / (((HEADER_BYTES % AXIS_BYTES) == 0) ? 1 : 0);

  logic busy_reg;
  logic [$clog2(HEADER_BYTES+1)-1:0] byte_idx_reg;
  ...
  assign m_axis_tlast = busy_reg && packet_last_reg && (byte_idx_reg == (HEADER_BYTES - AXIS_BYTES));

  always_comb begin
    m_axis_tdata = '0;
    remain_bytes = HEADER_BYTES - byte_idx_reg;
    ...
    for (byte_idx = 0; byte_idx < AXIS_BYTES; byte_idx = byte_idx + 1) begin
      if ((byte_idx_reg + byte_idx) < HEADER_BYTES) begin
        m_axis_tdata[(byte_idx*8) +: 8] = i_header_flat[((byte_idx_reg + byte_idx)*8) +: 8];
      end
    end
  end
    ...
    if (!rst_n) begin
      ...
      if (!busy_reg) begin
        if (i_start) begin
          busy_reg <= 1'b1;
          byte_idx_reg <= '0;
          packet_last_reg <= i_header_is_packet_last;
        end
      end else if (m_axis_tvalid && m_axis_tready) begin
        if ((byte_idx_reg + AXIS_BYTES) >= HEADER_BYTES) begin
          busy_reg <= 1'b0;
          byte_idx_reg <= '0;
          packet_last_reg <= 1'b0;
          o_done <= 1'b1;
