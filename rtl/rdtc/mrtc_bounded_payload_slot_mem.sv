module mrtc_bounded_payload_slot_mem #(
...
`ifdef RDTC_BOUNDED_ASIC_SRAM
  ...
  for (genvar lane = 0; lane < LANES; lane = lane + 1) begin : g_sram_lane
    mrtc_rdtc_block_1rw_256x32 u_sram (
      .clk0  (clk),
      .csb0  (~i_cmd_valid),
      .web0  (~i_cmd_write),
      .addr0 (i_cmd_addr),
      .din0  (i_wr_data[(lane*LANE_W) +: LANE_W]),
      .dout0 (macro_rd_data[(lane*LANE_W) +: LANE_W])
    );
  end
...
`ifndef RDTC_BOUNDED_ASIC_SRAM
  initial $fatal(1, "mrtc_bounded_payload_slot_mem requires a bounded ASIC storage profile");
`endif
`endif
