...
公开 RTL 使用单一 `clk` 和低有效异步 datapath reset `rst_n`。`i_clear_status` 只清 completion/counter 统计，不清 sticky fatal status、不替代 reset，也不应被用来中断正在握手的 AXI-Stream transaction。所有 `tvalid/tready` 传输都在 `clk` 上升沿完成。
