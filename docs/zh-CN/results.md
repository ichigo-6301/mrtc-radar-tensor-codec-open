...
| `register-expanded` | Nangate45 TT/1.1 V/25 C | DC matrix | 400/600 MHz 闭合；800 MHz WNS/TNS -0.14/-858.86 ns，因此选择 600 MHz mapped netlist | verified |
...
NanGate15 Liberty 使用 `1ps` 时间单位，DC profile 显式应用 `SDC_TIME_SCALE=1000.0`。45 nm register-expanded 后端使用已闭合的 600 MHz DC netlist，在 400 MHz 进行物理实现；handoff netlist、SDC 与 SPEF 的 SHA256 在 evidence 中一致记录。
