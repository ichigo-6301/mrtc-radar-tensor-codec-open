...
The buffered and Direct-AXIS wrappers were resynthesized with Design Compiler `O-2018.06-SP1`, one SHA256-bound Nangate45 typical DB, a common 315 MHz synchronous-boundary SDC, two Engines, register-expanded storage, `compile_ultra`, one design-rule repair pass, and retiming disabled. The reported runs at `as_run_flow_commit=db2660c` sourced the local `RDTC_DC_SETUP` required by that revision; the run script then verified the selected target DB hash. The current public reproduction runner was hardened after evidence collection: it binds the target and link library directly from the audited DB and does not execute a local setup Tcl. The four points were not rerun under this later setup-free runner, so that property is not attributed to the published metrics. The only intended architecture difference is that Direct removes the DDR feeder and per-Engine payload commit stores, reducing audited bulk storage from `180,224` to `32,768 bits`.
