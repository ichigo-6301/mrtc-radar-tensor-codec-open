...
| `sram-macro` | Nangate45/OpenRAM/OpenROAD/OpenRCX | Two `64x128 1RW1R` macros; fixed verified 333 MHz P&R, same-run SPEF, and internal PT timing closure point | Route DRC/antenna are 0/0; PT setup/hold WNS +0.57/+0.04 ns with zero constraint violations | Chip-level implementation verified; overall profile partial because the analytical SRAM model and macro DRC/LVS/PEX are not closed |
