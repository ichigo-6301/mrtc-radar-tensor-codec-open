...
class BoundedDirectModelsimRegressionTests(unittest.TestCase):
    ...
    def test_testbench_trace_is_opt_in_and_makefile_has_dedicated_targets(self):
        ...
        self.assertIn("trace_fifo_identity_valid", testbench)
        self.assertIn("u_dut.u_output_fifo.wr_ptr_reg", testbench)
        self.assertIn("u_dut.u_output_fifo.rd_ptr_reg", testbench)
        self.assertIn("u_dut.job_engine_reg[u_dut.output_rd_ptr_reg]", testbench)
        self.assertNotIn("out_tvalid ? (packet_count & 1)", testbench)
