...
当前公开发行是 **RDTC v1 lossless codec IP `register550-rc2`**。这是结构与复现性加固发行，不改变 RTL、reference behavior、码流、接口、寄存器映射或已发布实现指标。
...
| `rdtc_v1_register_ics55_ecos_preview` | planned | ICS55/ECOS preview | 尚无公开结果、evidence 或 implementation claim |
...
| Register-expanded 15/45 nm DC | verified | 400/600/700/800 MHz 矩阵有记录；45 nm 的 700 MHz 点闭合，800 MHz 未闭合 |
